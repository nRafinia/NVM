...
public class TreeViewBase<T> : ComponentBase
{
    ...
    /// </summary>
    [Parameter]
    public RenderFragment<ItemContent<T>> ItemTemplate { get; set; } =
        (ItemContent<T> value) => (builder) => builder.AddContent(0, @value.Item);
    ...
    /// </summary>
    [Parameter]
    public CheckboxFragment CheckboxTemplate { get; set; } = (value, indeterminate, valueChanged, disabled) => (builder) =>
    {
        ...
        builder.AddAttribute(3, nameof(DefaultCheckbox.ValueChanged), valueChanged);
    ...
    internal void UpdateSelection(T item, bool selected, bool indeterminate)
    {
        ...
        if (!selected || indeterminate)
        {
            ...
            InvokeAsync(async () =>
            {
                try
                {
                    if (SelectedItemsChanged is not null)
                    {
                        await SelectedItemsChanged.Value.InvokeAsync(SelectedItems);
                    }
                }
                catch
                {
